...
export async function assessReport(text, lang) {
  const { data, error } = await supabase.functions.invoke('report-assess', { body: { text, lang } })
  if (error) throw new Error(error.message || 'The assessment function failed.')
  if (data?.error) throw new Error(data.error)
  return data // { caps, evidence, model, truncated }
}

export async function saveReportCheck({ lang, companyName, sourceFilename, caps, evidence, aiModel }) {
  for (let attempt = 0; attempt < 5; attempt++) {
    const code = generateCode()
    const row = {
      code, lang, caps, evidence, ai_model: aiModel,
